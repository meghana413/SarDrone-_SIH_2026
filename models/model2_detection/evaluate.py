...
def evaluate(config_path: str | Path = "config/config.yaml", split: str = "val") -> dict[str, Any]:
    ...
    weights = config.get("model_path") or config["paths"].get("model_weights")
    if not weights:
        raise ValueError("a model_path is required")
    model = YOLO(weights)
    data = check_det_dataset(config["paths"]["data_yaml"])
    device = _resolve_device(config.get("device", config["model"]["device"]))
    imgsz = config.get("imgsz", config["model"]["image_size"])
    confidence = config.get("confidence", config["model"]["confidence"])
    metrics = model.val(data=config["paths"]["data_yaml"], imgsz=imgsz, device=device, split=split, plots=True, verbose=False)
    box = metrics.box
    names = data["names"]
    per_class = {}
    ...
    prediction_count = sum(len(result.boxes) for result in model.predict(source=str(image_dir), imgsz=imgsz, conf=confidence, device=device, save=False, verbose=False, stream=True))
    report: dict[str, Any] = {
        "split": split,

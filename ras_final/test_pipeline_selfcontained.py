...
def test_ras_final_has_no_legacy_payload_imports() -> None:
    root = pathlib.Path(__file__).resolve().parent
    for path in sorted(root.rglob("*.py")):
        if path.name == "test_pipeline_selfcontained.py":
            continue
        text = path.read_text(encoding="utf-8")
        assert "from payload" not in text
        assert "import payload" not in text
        assert "from lora_sender" not in text
        assert "import lora_sender" not in text

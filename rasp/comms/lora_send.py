...
class LoRaSender:
    ...
    def send_compact(self, compact: str, framing: bytes = b"_") -> None:
        """Transmit an already-formed five-character detection result.

        The default underscore separates fixed-width messages on the receiver.
        ``make_compact_output`` itself remains a pure five-character AI result.
        """
        if len(compact) != 5 or not compact.isdigit():
            raise ValueError("compact detection output must be exactly five digits")
        self.open()
        assert self._serial is not None
        packet = compact.encode("ascii") + framing
        written = self._serial.write(packet)
        if written != len(packet):
            raise IOError(f"serial write incomplete: {written}/{len(packet)} bytes")
